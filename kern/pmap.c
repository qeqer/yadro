...
boot_alloc(uint32_t n)
{
	...
	if (n > 0) {
		nextfree += (uintptr_t)ROUNDUP((char *) n, PGSIZE); 
...
mem_init(void)
{
	...
	//    - envs itself -- kernel RW, user NONE
	// LAB 12: Your code here.
	boot_map_region(kern_pgdir, UVSYS, ROUNDUP(NVSYSCALLS * sizeof(*vsys), PGSIZE),
		PADDR(vsys), PTE_U);
	//////////////////////////////////////////////////////////////////////
...
page_init(void)
{
	...
	int index = PGNUM(PADDR(boot_alloc(0)));
	// find number of first free page
	//(boot_alloc(0) --> If n==0, returns the address of the next free page without allocating
	// find PGNUM(PADDR(boot_alloc(0))) and go on by all page
	for (i = index; i < npages; i++) {
		pages[i].pp_ref = 0;
		pages[i].pp_link = page_free_list;
		page_free_list = &pages[i];
	}

	for (i = npages_basemem; i < index; i++)
		pages[i].pp_ref = 1;
}
...
page_alloc(int alloc_flags) {
	...
	page_free_list = tmp->pp_link;
	tmp->pp_link = NULL;

	if (ALLOC_ZERO == alloc_flags) {
		char *va = page2kva(tmp);
		memset(va, 0, PGSIZE);
	} 
...
//
// По указателю на каталог страниц (pgdir) возвращает запись в таблице страниц для линейного адреса va. 
// Соответствующая запись может еще не существовать, в таком случае, если create == false, возвращает NULL, 
// иначе выделяет новую таблицу страниц с помощью page_alloc. Увеличивает счетчик ссылок на страницу. 
// Флаги должны быть установлены в perm|PTE_P.
pte_t *
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	for (i = 0; i < size; i += PGSIZE)
		*pgdir_walk(pgdir, (void *) va + i, 1) = (pa + i) | perm | PTE_P; // get record from page table by address va + i and map pa + i with flags to it
}
